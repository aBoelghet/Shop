...
class HADropDown: UIView {
    ...
    fileprivate var itemHeight1 = 35.0
    ...
    var font : UIFont {
        set {
            selectedFont = ((UIDevice.current.userInterfaceIdiom == .pad) ? UIFont(name: "HelveticaNeue-Medium", size: 14) :UIFont(name: "HelveticaNeue-Medium", size: 12))!//newValue
            label.font = (UIDevice.current.userInterfaceIdiom == .pad) ? UIFont(name: "HelveticaNeue-Medium", size: 14) :UIFont(name: "HelveticaNeue-Medium", size: 12)//selectedFont
        }
        get {
            return selectedFont
        }
    ...
    override func prepareForInterfaceBuilder() {
        label.frame = CGRect(x: 0, y: 0, width: self.frame.width, height: self.frame.height)
        font = ((UIDevice.current.userInterfaceIdiom == .pad) ?UIFont(name: "HelveticaNeue-Medium", size: 14) :UIFont(name: "HelveticaNeue-Medium", size: 12))! //UIFont(descriptor: font.fontDescriptor, size: titleFontSize)
        label.font = (UIDevice.current.userInterfaceIdiom == .pad) ?UIFont(name: "HelveticaNeue-Medium", size: 14) :UIFont(name: "HelveticaNeue-Medium", size: 12)
    ...
    @objc private func didTap(gesture: UIGestureRecognizer) {
        ...
        if !isCollapsed {
            let height : CGFloat = CGFloat(items.count > 3 ? itemHeight*3 : itemHeight*Double(items.count))
...
//        selectedIndex = indexPath.row
        ...
        collapseTableView()
        if delegate != nil {
            delegate.didSelectItem(dropDown: self, at: indexPath.row)
        }
    }
