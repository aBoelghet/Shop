...
open class GNAMenuView: UIView {
    ...
    public func showMenuView(inView view: UIView, atPoint: CGPoint) {
    ...
    public func refreshTheViews(_ point:CGPoint) {
        
        let tempDirection = currentDirection
        
        touchPoint = point
        touchPointImage.center = point
        currentDirection = calculateDirections(menuItemsArray[0].frame.width)
        ...
        // Comparing only second item which will be left, right and middle
        if currentDirection!.1 != tempDirection!.1 {
            if let theView = self.superview {
    ...
    public func dismissMenuView() {
        detectPoint(touchPoint!, action: { (menuItem: GNAMenuItem) in
    ...
    private func determineHorisontalDirection(_ size: CGFloat, superViewFrame: CGRect) -> Direction {
        ...
        let isRightBorderOfScreen = tPoint.x > UIScreen.main.bounds.width / 2
        let isLeftBorderOfScreen = tPoint.x < UIScreen.main.bounds.width / 2
    ...
    private func anglesForDirection() {
        ...
        case (.down, .left), (.down, .middle):
            negativeQuorterAngles(startAngle: 135)            // Changed by Venu  earlier 90
            break
        case (.down, .right):
            positiveQuorterAngle(startAngle: 315)            // Changed by Venu  earlier 0
            break
        case (.middle, .left), (.middle, .middle), (.up, .left), (.up, .middle):
            negativeQuorterAngles(startAngle: 135)          // Changed by Venu  earlier 180
            break
    ...
    private func negativeQuorterAngles(startAngle: CGFloat) {
        ...
        menuItemsArray.forEach({ item in
            ...
            item.angle = (angle - angleCoef * index) / 180 * .pi
            //            angleCoef = CGFloat(Int(angleCoef + 1))
        })
    }
    ...
    private func positiveQuorterAngle(startAngle: CGFloat) {
        menuItemsArray.forEach({ item in
            let index = CGFloat(menuItemsArray.index(of: item)!)
            item.angle = (startAngle + angleCoef * index) / 180.0 * .pi
            //            angleCoef = CGFloat(Int(angleCoef + 1))
        })
    }
...
//    var previousY:CGFloat = 0               //Added
//    let gapForPopUps:CGFloat = 12           //Added
