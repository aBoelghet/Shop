...
@objc open class ACFloatingTextfield: UITextField {
    ...
    let padding = UIEdgeInsets(top: 10, left: 5, bottom: 0, right: 5)
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel(){
        ...
        let leadingConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .leading, relatedBy: .equal, toItem: self, attribute: .leading, multiplier: 1, constant: self.padding.left)
...
extension ACFloatingTextfield {
    ...
    func validateTextField (type:Validations, textField:ACFloatingTextfield) {
        
        let someBoolean = checkValidations(type: type, textField: textField)
        ...
        if isActive {
            if someBoolean {
                colorToApply = UIColor.green
                
            } else {
                colorToApply = UIColor.red
            }
        } else {
            if someBoolean {
                colorToApply = UIColor.black
//                colorToApply = UIColor.green
        ...
        textField.selectedLineColor = colorToApply
        textField.lineColor = colorToApply
    }
    
    func checkValidations(type:Validations, textField:ACFloatingTextfield)->Bool {
        
        var someBoolean = Bool()
        
        switch type {
        case .ValidateEmail: someBoolean = (textField.text?.isValidEmail())!
            break
        case .ValidatePhone: someBoolean = (textField.text?.isValidPhone())!
            break
        case .ValidatePassword: someBoolean = (textField.text?.isValidPassword())!
            break
