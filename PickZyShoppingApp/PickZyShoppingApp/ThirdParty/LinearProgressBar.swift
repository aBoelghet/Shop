...
open class LinearProgressBar: UIView {
    ...
    public init () {
        super.init(frame: CGRect(origin: CGPoint(x: 0,y : GSConstant.progressYaxis), size: CGSize(width: screenSize.width, height: 0)))
        self.backgroundProgressBarColor = GSConstant.progressBarBGColor
        self.progressBarColor = GSConstant.progressBarColor
        self.heightForLinearBar = GSConstant.progressBarHeight
            ...
            UIView.animate(withDuration: 0.5, delay:0, options: [], animations: {
                self.frame = CGRect(x: 0, y: self.frame.origin.y, width: self.widthForLinearBar, height: self.heightForLinearBar)
                    ...
                    if let topController = self.getTopViewController() {
                        // Added by Venu
                        if let unwrappedMenuBar = menuBar, unwrappedMenuBar.isDescendant(of: topController.view) {
                            topController.view.bringSubview(toFront: unwrappedMenuBar)
                        }
                        // Added by Venu
                    }
    ...
    open func progressBarPositionChange() {
        self.frame = CGRect(origin: CGPoint(x: 0,y : GSConstant.progressYaxis), size: CGSize(width: screenSize.width, height: 0))
    }
