    ...
    func configureTheCell(items:GSProductsList, selectedSizeClass:CGFloat) {
        ...
        if let imageArr = items.productInfo!.images {
            if imageArr.count > 0 {
                ...
                itemsImageView.sd_setImage(with: URL(string: imageUrlString), placeholderImage: #imageLiteral(resourceName: "blurImage"), options: .progressiveDownload) { ( image, error, _ , _ ) in
                    if error == nil {
