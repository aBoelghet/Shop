...
extension GSNotificationsViewController : UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate func discountNotifications (at indexPath: IndexPath, notification: GSNotificationOffersListData) ->  GSPromoCodeTableViewCell {
        ...
        cell.store_imgView.sd_setImage(with: URL(string: APIurl.baseURL + APIurl.subURL.viewPromoCodeStoreImage + (promoCodeAtIndex.shopIcon ?? "") + imgHeight), placeholderImage: #imageLiteral(resourceName: "blurImage"), options: .progressiveDownload, progress: nil) { (image, error, _ , _ ) in
