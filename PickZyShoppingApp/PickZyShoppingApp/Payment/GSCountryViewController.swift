...
//            let imageUrl = URL(string: "http://www.geognos.com/api/en/countries/flag/\(itemAtIndex.alpha2Code).png")
            ...
            cell.countryIcon_imageView.sd_setImage(with: imageUrl, placeholderImage: nil, options: .progressiveDownload, completed: nil)
...
extension GSCountryViewController {
    ...
    func countryApi() {
        ...
        APIHandler.NetworkSetupRequest(method: .get, params: nil, urlString: APIurl.countryListURL, headers: nil, withLoader:true) { (response, error) in
