...
//            expDateTxtField.text    = paymentParams.expiryMonth! + "/" + expiryYear.substring(from:expiryYear.index(expiryYear.endIndex, offsetBy: -2))
    ...
    private func applyColors() {
        ...
        for tf in tfArray {
            ...
            tf?.setValue(UIColor(hexString: defaultTheme.addNewCard_placeHolder), forKeyPath: "_placeholderLabel.textColor")
        }
...
extension GSAddCardViewController:CountrySelectedDelegate {
    ...
    func countrySelectedWith(_ name: String, imageUrl: String) {
        ...
        selCountry_imageView.sd_setImage(with: URL(string: imageUrl), placeholderImage: nil, options: .progressiveDownload, completed: nil)
    }
}
